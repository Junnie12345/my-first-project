...
delta_stats <- delta_data %>%
  ...
  select(measure_full, 
         any_of(c("diff_mean_A", "diff_SEM_A", "diff_mean_B", "diff_SEM_B")))
...
for(i in 1:nrow(combinations)) {
  ...
  # 1. Group A 組內差異 (Pre vs Post) -> *
  dat_A <- sub_dat %>% filter(Group == "A", Is_Paired == TRUE)
  p_within_A <- NA
  if(nrow(dat_A) >= 2 && n_distinct(dat_A$Trail) == 2) {
    try({ p_within_A <- wilcox.test(score ~ Trail, data = dat_A, paired = TRUE)$p.value }, silent=T)
  }
  
  # 2. Group B 組內差異 (Pre vs Post) -> #
  dat_B <- sub_dat %>% filter(Group == "B", Is_Paired == TRUE)
  p_within_B <- NA
  if(nrow(dat_B) >= 2 && n_distinct(dat_B$Trail) == 2) {
    try({ p_within_B <- wilcox.test(score ~ Trail, data = dat_B, paired = TRUE)$p.value }, silent=T)
  }
  
  # 3. Delta 組間差異 (Delta A vs Delta B) -> $
  p_delta_bet <- NA
  if(nrow(sub_delta) >= 2 && n_distinct(sub_delta$Group) == 2) {
    try({ p_delta_bet <- wilcox.test(Diff ~ Group, data = sub_delta)$p.value }, silent=T)
  }
  
  # 4. Pre/Post 同時間點組間差異 (Pre A vs B, Post A vs B) -> $ for Pre/Post plots
  p_pre_bet <- NA
  try({ p_pre_bet <- wilcox.test(score ~ Group, data = sub_dat %>% filter(Trail == "Pre"))$p.value }, silent=T)
  
  p_post_bet <- NA
  try({ p_post_bet <- wilcox.test(score ~ Group, data = sub_dat %>% filter(Trail == "Post"))$p.value }, silent=T)
...
get_y_unit_simple <- function(metric_name) {
  ...
  if (clean %in% c("TP", "VLF", "LF", "HF", "alleeg")) return("(ms\u00B2)")
  if (clean %in% c("LF_HF", "LF_HF_ratio")) return("(Ratio)")
  if (clean %in% c("SD", "SDNN", "RMSSD", "RR", "SDANN", "SDSD")) return("(ms)")
  if (clean == "MPF") return("(Hz)")
  if (clean == "mSpO2") return("(%)")
  if (clean == "n") return("(Count)")
  return("") 
}
...
for (m in all_measures_full) {
  ...
  p <- ggplot(sub_data, aes(x = Trail, y = mean, group = Group, color = Group)) +
    ...
    theme_classic() + 
    theme(plot.title = element_text(face="bold", size=14, hjust=0.5), axis.text=element_text(size=12, color="black"),
...
for (metric in all_metrics) {
  ...
  if(nrow(dat_delta) > 0) {
    ...
    # A 組 (*)
    anno_A <- dat_delta_p %>%
      filter(Group == "A", p_within_A < 0.05) %>%
      mutate(
    ...
    # B 組 (#)
    anno_B <- dat_delta_p %>%
      filter(Group == "B", p_within_B < 0.05) %>%
      mutate(
        x_pos = as.numeric(Stage) + 0.2,
        base_y = ifelse(mean+se > 0, mean+se, 0),
        y_pos = base_y + buffer_small,
    ...
    max_anno_y <- -Inf
    if(nrow(anno_A) > 0) max_anno_y <- max(max_anno_y, max(anno_A$y_pos))
    if(nrow(anno_B) > 0) max_anno_y <- max(max_anno_y, max(anno_B$y_pos))
    ...
    # 加上標記
    if(nrow(anno_A) > 0)   p_delta_final <- p_delta_final + geom_text(data=anno_A, aes(x=x_pos, y=y_pos, label=label), inherit.aes=F, size=6, fontface="bold")
    if(nrow(anno_B) > 0)   p_delta_final <- p_delta_final + geom_text(data=anno_B, aes(x=x_pos, y=y_pos, label=label), inherit.aes=F, size=5, fontface="bold", color="black")
