...
# ==== 0. 載入必要套件 (自動安裝缺失套件) ====
# ==============================================================================
pkg_list <- c("tidyverse", "openxlsx", "readxl", "rstatix", "ez", "geepack", 
...
merged_data <- data_clean %>%
  ...
  mutate(
    Group_Label = factor(case_when(
      Group %in% c("0", "A", "a", "Control") ~ "A",
      Group %in% c("1", "B", "b", "Exper")   ~ "B",
      TRUE ~ NA_character_
    ), levels = c("A", "B")),
...
data_3pt <- long_data %>% group_by(Group, measure, ID) %>% filter(n() == 3) %>% ungroup()

if(nrow(data_3pt) > 0) {
  friedman_results <- data_3pt %>% group_by(Group, measure) %>%
    do({ res <- tryCatch({ friedman_test(data = ., score ~ Trail | ID) }, error = function(e) NULL); if(is.null(res)) data.frame() else as.data.frame(res) }) %>% ungroup()
  
  anova_results <- data_3pt %>% group_by(Group, measure) %>%
    do({ res <- tryCatch({ anova_test(data = ., dv = score, wid = ID, within = Trail) }, error = function(e) NULL); if(is.null(res)) data.frame() else as.data.frame(res) }) %>% ungroup()
  ...
  for (m in unique(data_3pt$measure)) {
    for (g in unique(data_3pt$Group)) {
      ...
      if (nrow(sub) >= 6 && sd(sub$score, na.rm=T) > 0) {
        res <- tryCatch({ pairwise_wilcox_test(sub, score ~ Trail, paired=T, p.adjust.method="bonferroni") %>% mutate(measure=m, Group=g) }, error=function(e) NULL)
...
# 4.2 Delta
delta_wide <- data_2pt %>% select(ID, Group, Trail, measure, score) %>% pivot_wider(names_from = Trail, values_from = score)
if ("Pre" %in% names(delta_wide) && "Post" %in% names(delta_wide)) {
  delta_data <- delta_wide %>% mutate(Diff = Post - Pre) %>% filter(!is.na(Diff))
  desc_delta <- delta_data %>% group_by(Group, measure) %>% summarise(n=n(), Delta_Mean=mean(Diff,na.rm=T), Delta_SEM=sd(Diff,na.rm=T)/sqrt(n), .groups="drop")
} else {
  delta_data <- data.frame(); desc_delta <- data.frame()
}

# 4.3 P-value Loop
p_list_np <- list(); p_list_p <- list()
for(m in unique(data_2pt$measure)) {
  sub <- data_2pt %>% filter(measure==m); d_sub <- if(nrow(delta_data)>0) delta_data %>% filter(measure==m) else data.frame()
  ...
  # Intra
  for(g in c("A","B")) {
    tmp <- sub %>% filter(Group==g) %>% arrange(ID, Trail)
    ids <- tmp %>% count(ID) %>% filter(n==2) %>% pull(ID); tmp <- tmp %>% filter(ID %in% ids)
    if(length(ids)>=2) {
      vp <- tmp$score[tmp$Trail=="Pre"]; vt <- tmp$score[tmp$Trail=="Post"]
      try({ res_p[[paste0("p_Within_",g)]] <- t.test(vp, vt, paired=T)$p.value }, silent=T)
      try({ res_np[[paste0("p_Within_",g)]] <- wilcox.test(vp, vt, paired=T)$p.value }, silent=T)
    }
  }
  # Inter
  for(tm in c("Pre","Post")) {
    ...
    if(n_distinct(dt$Group)==2 && min(table(dt$Group))>=2) {
      try({ res_p[[paste0("p_Bet_",tm)]] <- t.test(score~Group, data=dt)$p.value }, silent=T)
      try({ res_np[[paste0("p_Bet_",tm)]] <- wilcox.test(score~Group, data=dt)$p.value }, silent=T)
    }
  }
  # Delta
  if(nrow(d_sub)>0 && n_distinct(d_sub$Group)==2 && min(table(d_sub$Group))>=2) {
    try({ res_p$p_Bet_Delta <- t.test(Diff~Group, data=d_sub)$p.value }, silent=T)
    try({ res_np$p_Bet_Delta <- wilcox.test(Diff~Group, data=d_sub)$p.value }, silent=T)
  }
  p_list_np[[m]] <- res_np; p_list_p[[m]] <- res_p
}
df_p_np <- bind_rows(p_list_np); df_p_p <- bind_rows(p_list_p)
...
if(nrow(desc_delta)>0) {
  ...
  final_np <- tbl_pp %>% left_join(tbl_d, by="measure") %>% left_join(df_p_np, by="measure")
  final_p <- tbl_pp %>% left_join(tbl_d, by="measure") %>% left_join(df_p_p, by="measure")
} else {
...
for(m in measures_to_analyze) {
  ...
  if(nrow(tmp) < 10) next
  mod <- tryCatch({ suppressWarnings(geeglm(score ~ Group * Trail, data = tmp, id = ID, corstr = "ar1")) }, 
                  error = function(e) tryCatch({ suppressWarnings(geeglm(score ~ Group * Trail, data = tmp, id = ID, corstr = "exchangeable")) }, error=function(e2) NULL))
  if(!is.null(mod)) {
    try({
      emm <- emmeans(mod, ~ Group | Trail)
      res <- pairs(emm, reverse=TRUE) %>% as.data.frame() %>% mutate(measure = m)
      gee_res_list[[m]] <- res
    }, silent=TRUE)
  }
}
...
for(m in measures_to_analyze) {
  ...
  mod <- tryCatch({ suppressMessages(suppressWarnings(lmer(score ~ Group * Trail + Age + BMI + PSQI_baseline + (1|ID), data = tmp))) }, error = function(e) NULL)
  
  if(!is.null(mod)) {
    res_tidy <- tryCatch({ broom.mixed::tidy(mod, conf.int=TRUE, effects="fixed") %>% mutate(measure=m) }, error=function(e) NULL)
    if(is.null(res_tidy)) {
      try({
        coefs <- summary(mod)$coefficients
        res_tidy <- as.data.frame(coefs) %>% rownames_to_column("term") %>% 
          rename(estimate=Estimate, std.error=`Std. Error`, statistic=`t value`, p.value=`Pr(>|t|)`) %>% mutate(measure=m)
      }, silent=TRUE)
    }
...
addWorksheet(wb, "1_三時點_Friedman"); writeData(wb, "1_三時點_Friedman", friedman_results)
addWorksheet(wb, "1_三時點_ANOVA"); writeData(wb, "1_三時點_ANOVA", anova_results)
addWorksheet(wb, "1_三時點_Posthoc"); writeData(wb, "1_三時點_Posthoc", posthoc_wilcox_3pt)
addWorksheet(wb, "2_PrePost_有母數"); writeData(wb, "2_PrePost_有母數", final_p); conditionalFormatting(wb, "2_PrePost_有母數", cols=which(grepl("p_", names(final_p))), rows=2:200, rule="<0.05", style=sig)
addWorksheet(wb, "3_PrePost_無母數"); writeData(wb, "3_PrePost_無母數", final_np); conditionalFormatting(wb, "3_PrePost_無母數", cols=which(grepl("p_", names(final_np))), rows=2:200, rule="<0.05", style=sig)
addWorksheet(wb, "4_GEE_PostHoc"); if(nrow(df_gee_posthoc)>0) writeData(wb, "4_GEE_PostHoc", df_gee_posthoc)
addWorksheet(wb, "5_LMM_Adjusted"); if(nrow(df_lmm_adj)>0) writeData(wb, "5_LMM_Adjusted", df_lmm_adj)
...
get_y_label <- function(m_name) {
  if(grepl("_min", m_name) || m_name %in% c("TRT", "TST", "SL", "WASO", "REM", "N1", "N2", "N3")) return("Time (min)")
  if(grepl("%", m_name) || m_name %in% c("SE")) return("Percentage (%)")
  if(m_name %in% c("ArousalIndex", "ODI", "AHI", "AI", "HI", "REM_AHI", "NREM_AHI", "Supine_AHI", "NonSupine_AHI")) return("Frequency (times/hr)")
  if(grepl("HR", m_name)) return("Heart Rate (bpm)")
  return("Score")
}
...
for(m in measures_to_analyze) {
  ...
  anno_df <- data.frame()
  if(nrow(stats) > 0) {
    if(!is.na(stats$p_Within_A) && stats$p_Within_A < 0.05) anno_df <- rbind(anno_df, data.frame(Trail="Post", Y=sum_sub$Mean[sum_sub$Group=="A" & sum_sub$Trail=="Post"] + sum_sub$SEM[sum_sub$Group=="A" & sum_sub$Trail=="Post"], Label="*", Group="A"))
    if(!is.na(stats$p_Within_B) && stats$p_Within_B < 0.05) anno_df <- rbind(anno_df, data.frame(Trail="Post", Y=sum_sub$Mean[sum_sub$Group=="B" & sum_sub$Trail=="Post"] + sum_sub$SEM[sum_sub$Group=="B" & sum_sub$Trail=="Post"], Label="#", Group="B"))
  ...
  p1 <- ggplot(sum_sub, aes(x = Trail, y = Mean, group = Group, color = Group, shape = Group)) +
    ...
    labs(title = m, subtitle = "Mean ± SEM (*:A intra, #:B intra, $:Inter)", y = y_lab, x = NULL) +
