...
wide_to_long <- function(df, id_vars = c("ID", "Name", "Group"), value_bases = outcome_base_vars) {
  ...
  for (base in value_bases) {
    ...
    if (length(cols) == 0) { message("  [警告] 找不到變數: ", base); next }
    temp <- df %>%
...
long <- long %>%
  ...
  mutate(Group = case_when(Group == "0" ~ "A", Group == "1" ~ "B", tolower(Group) == "control" ~ "A", tolower(Group) == "exp" ~ "B", TRUE ~ Group)) %>%
...
group_weekly_descriptive <- function(long_df, vars = outcome_base_vars) {
  des_list <- list()
  for (v in vars) {
    temp <- long_df %>% group_by(Group, week_numeric) %>%
      ...
      mutate(variable = v) %>% select(variable, everything())
    des_list[[v]] <- temp
  }
  bind_rows(des_list)
}
...
normality_tests_grouped <- function(long_df, vars = outcome_base_vars, alpha = 0.05) {
  results <- data.frame()
  for (v in vars) {
    temp <- long_df %>% filter(!is.na(.data[[v]])) %>% group_by(Group, week_numeric) %>%
      summarise(n = n(), shapiro_p = tryCatch({ if(n>=3 && sd(.data[[v]])>0) round(shapiro.test(.data[[v]])$p.value, 4) else NA }, error=function(e) NA), .groups="drop") %>%
...
cor_results <- tryCatch({
  df_cor <- long %>% select(all_of(outcome_base_vars)) %>% filter(complete.cases(.))
...
for (i in seq_along(outcome_base_vars)) {
  ...
  df_sub <- long %>% select(ID, Group, week_factor, week_numeric, score = all_of(outcome)) %>% filter(!is.na(score)) %>% arrange(ID, week_numeric)
  
  if (nrow(df_sub) >= 20) {
    m_ar1  <- tryCatch({ geeglm(f_gee, data=df_sub, id=ID, family=gaussian, corstr="ar1") }, error=function(e) NULL)
    m_exch <- tryCatch({ geeglm(f_gee, data=df_sub, id=ID, family=gaussian, corstr="exchangeable") }, error=function(e) NULL)
    
    q_ar1  <- if(!is.null(m_ar1)) QIC(m_ar1)[1] else Inf
    q_exch <- if(!is.null(m_exch)) QIC(m_exch)[1] else Inf
    
    best_mod <- NULL; best_cor <- "None"
    if (q_ar1 != Inf && (q_exch == Inf || q_ar1 <= q_exch)) { best_mod <- m_ar1; best_cor <- "ar1" } else if (q_exch != Inf) { best_mod <- m_exch; best_cor <- "exchangeable" }
...
wb <- createWorkbook()
addWorksheet(wb, "Descriptive"); writeData(wb, "Descriptive", desc_data)
addWorksheet(wb, "Normality"); writeData(wb, "Normality", norm_results)
if(!is.null(cor_results)) { addWorksheet(wb, "Correlation"); writeData(wb, "Correlation", cor_results) }
if(nrow(gee_best_res$summary)>0) {
  addWorksheet(wb, "GEE_Best_Main"); writeData(wb, "GEE_Best_Main", gee_best_res$summary)
  addWorksheet(wb, "GEE_Best_Group"); writeData(wb, "GEE_Best_Group", gee_best_res$pw_group)
  addWorksheet(wb, "GEE_Best_Time"); writeData(wb, "GEE_Best_Time", gee_best_res$pw_time)
}
if(nrow(gee_spec_res$summary)>0) {
  addWorksheet(wb, "GEE_AR1_Main"); writeData(wb, "GEE_AR1_Main", gee_spec_res$summary)
  addWorksheet(wb, "GEE_AR1_Group"); writeData(wb, "GEE_AR1_Group", gee_spec_res$pw_group)
  addWorksheet(wb, "GEE_AR1_Time"); writeData(wb, "GEE_AR1_Time", gee_spec_res$pw_time)
}
...
for (i in seq_along(vars_to_plot)) {
  ...
  if(nrow(df_s)==0) { setTxtProgressBar(pb_line, i); next }
  
  df_s$label_A <- NA; df_s$label_B <- NA; df_s$label_AB <- NA
  ...
  if(use_ar1) {
    ...
    for(k in 1:nrow(df_s)) {
      w <- df_s$week_numeric[k]; g <- as.character(df_s$Group[k])
      if(w > base_w) {
        res <- stats_t %>% filter(grepl(paste0(w), contrast) & grepl(paste0(base_w), contrast) & Group == g)
        if(nrow(res)>0 && res$p.value[1]<0.05) { if(g=="A") df_s$label_A[k] <- "*" else df_s$label_B[k] <- "#" }
      }
    }
    # Inter
    for(k in 1:nrow(df_s)) {
  ...
  ymin <- min(df_s$avg - df_s$se, na.rm=T); ymax <- max(df_s$avg + df_s$se, na.rm=T)
  if(ymin==ymax) { ymin <- ymin-0.1; ymax <- ymax+0.1 }
  ...
  p_anno <- p +
    geom_text(aes(label=label_A, y=avg+se), vjust=-0.5, size=7, show.legend=F, na.rm=T) +
    geom_text(aes(label=label_B, y=avg+se), vjust=-0.5, size=6, show.legend=F, na.rm=T) +
...
plot_pair <- function(data, vars, title_suffix) {
  df_sub <- data %>% select(Group, all_of(vars)) %>% na.omit() %>% mutate(Group = factor(Group, levels=c("A","B")))
  if(nrow(df_sub) > 0) {
    my_colors_pair <- c("#00BFC4", "#F8766D")
    p <- ggpairs(
      df_sub, columns = 2:ncol(df_sub),
...
for (i in seq_along(trend_vars)) {
  ...
  if(nrow(plot_df) < 10) { setTxtProgressBar(pb_trend, i); next }
  ...
  p_trend <- ggplot(plot_df, aes(x=week_numeric, y=value, color=Group, fill=Group)) +
    ...
    scale_color_manual(values=c("A"="#00BFC4", "B"="#F8766D")) +
    scale_fill_manual(values=c("A"="#00BFC4", "B"="#F8766D")) +
